...
public class PostHandlerV3 implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            if (request.getUnita1() == null || request.getUnita1().trim().isEmpty()) {
                inviaErrore(exchange, 400, "Operatore mancante o vuoto");
                return;
            }
            
            if (request.getUnita2() == null || request.getUnita2().trim().isEmpty()) {
                inviaErrore(exchange, 400, "Operatore mancante o vuoto");
                return;
            }
            
            // Esegue il calcolo
            double risultato = ConversioneService.converti(
                request.getUnita1(),
                request.getUnita2(),
                request.getValore()
            );
